        ...
        <section class="col-12">
            <section class="main-body-container">
                ...
                <section class="d-flex justify-content-between align-items-center mt-4 mb-3 border-bottom pb-2">
                    <div></div>
                    <form action="{{ route('admin.order.filter.index') }}" method="GET" class="max-width-16-rem">
                        <input type="text" class="form-control form-control-sm form-text" name="search"
                            placeholder="جستجو" value="{{ request()->search }}">
                        <input type="text" name="filter" value="{{ request()->filter }}" class="d-none">
                            ...
                            @forelse ($orders as $order)
                                @if ($order->products()->where('seller_id', auth()->user()->id)->count() != 0 || auth()->user()->hasRole('admin'))
                                    <tr>
                                        ...
                                        <td><strong class="text-success">{{ priceFormat($order->final_price) }}</strong>
                                            تومان
                                        </td>
                                        <td>
